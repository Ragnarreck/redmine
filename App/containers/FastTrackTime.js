...
export default class FastTrackTime extends React.Component {
    state = {
        issues: this.props.issues || [],
        currentIssue: this.props.currentIssue || {},
        isLoading: !this.props.issues,
        showDatePicker: false,
        timeSheetInfo: { spentOn: moment(), hours: '', activity: '', comments: '' },
        activityOptions: [
            { value: 'Development', index: 9 },
            { value: 'Research', index: 10 },
            { value: 'Design', index: 8 },
            { value: 'Documentation', index: 11 },
            { value: 'Management', index: 12 },
        ],
    };

    componentWillMount() {
        return !this.state.issues.length &&
            getIssuesByProjectId(this.props.projectId)
